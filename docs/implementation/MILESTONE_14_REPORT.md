...
Playwright e2e for tasks and notifications added in this remediation pass:

* `apps/frontend/e2e/tasks.spec.ts` — real backend + real frontend (`/tasks` page). Covers: server-side scope (physician2 IM does not see physician1 Card tasks), foreign task → 404, acknowledge (assigned → in_progress) verified via API state, complete (in_progress → completed), invalid repeat completion → 409, reassignment to same-department nurse + `task_assignment` notification generated, cross-department reassignment denied.
* `apps/frontend/e2e/notifications.spec.ts` — real backend. Covers: recipient scoping, critical_lab_alert priority + PHI-free body, foreign notification → 404, acknowledge (dispatched → acknowledged), repeat acknowledge → 409, deterministic critical notification generated by the production diagnostics → critical-result flow (no LLM).

Critical notification coverage is also retained by the existing `diagnostics.test.ts` integration suite (real DB, real transaction, deterministic critical-value evaluator).

## Tests

* `pnpm -r run test` → shared 51 passed, frontend 58 passed, backend 666 passed.
...
PASS — full monorepo suite green (775 tests total: 51 shared + 58 frontend + 666 backend).
...
1. Playwright e2e for task/notification flows — resolved in this remediation pass.

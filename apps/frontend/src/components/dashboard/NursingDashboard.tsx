...
import { Stethoscope, CheckSquare, FlaskConical, AlertOctagon, RefreshCw, ArrowUpRight, Check } from 'lucide-react';
...
export function NursingDashboard() {
  ...
  return (
    <div className={styles.dashboardContainer}>
      <header className={styles.greetingBanner}>
        <div>
          <h1 className={styles.greetingTitle}>
            ...
            Nursing Station — {user?.firstName ? `${user.firstName} ${user.lastName ?? ''}` : 'Nurse'}
          </h1>
          <p className={styles.greetingSubtitle}>
            {now.toLocaleDateString(undefined, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
            {' · '}
            {ROLE_DISPLAY_NAMES.nurse} Workspace
          </p>
        </div>
        <div className={styles.greetingMeta}>
          <span className={styles.livePill}><span className={styles.liveDot} aria-hidden="true" />Ward Live</span>
      ...
      {/* Metrics */}
      <section aria-label="Nursing metrics" className={styles.metricRow}>
        <MetricCard
          label="Active Patients / Consultations"
          icon={<Stethoscope size={16} aria-hidden="true" />}
          tone="primary"
          href="/encounters"
          value={loading ? '—' : encounters.length}
          hint="Active in ward right now"
        />
        <MetricCard
          ...
          hint="Care instructions and vitals entry"
        />
        <MetricCard
          label="Specimen Collections Due"
          icon={<FlaskConical size={16} aria-hidden="true" />}
          tone="warning"
          href="/diagnostics"
          value={loading ? '—' : pendingSpecimens}
          hint="Lab samples to collect from patients"
        />
        <MetricCard
          ...
          hint={criticalItems.length === 0 ? 'No unacknowledged panics' : 'Immediate clinician review required'}
        />
      </section>
      ...
      <Card elevation="xs" padding="none" className={styles.tableCard}>
        <div className={styles.sectionCardHeader}>
          <div className={styles.sectionHeaderTitle}>
            <h3>Nursing Work Queue</h3>
            <p>Direct bedside and procedural tasks</p>
          </div>
        ...
        {loading ? (
          ...
          <CardContent>
            <p className={styles.quietEmpty}>No pending nursing tasks. The ward queue is up to date.</p>
          </CardContent>
        ) : (
          <Table ariaLabel="Nursing Tasks">
            <THead>
              <tr>
                <TH>Task Title</TH>
                <TH>Type</TH>

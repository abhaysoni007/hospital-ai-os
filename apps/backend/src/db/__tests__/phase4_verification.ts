...
async function cleanup() {
  if (cleanupStaffId) {
    try {
      ...
      console.log(
        '  [cleanup] Cannot delete test staff (referenced by audit_events — correct by design)',
      );
    }
  }
}
...
async function testUpdateRejected() {
  ...
  try {
    ...
    if (!row) {
      console.log('  ⚠ SKIP — no audit events to update');
      return;
    }
    ...
    const isExpected =
      causeMsg.includes('append-only') ||
      causeMsg.includes('strictly prohibited') ||
      msg.includes('append-only') ||
      msg.includes('strictly prohibited');
    if (isExpected) {
...
async function testDeleteRejected() {
  ...
  try {
    ...
    if (!row) {
      console.log('  ⚠ SKIP — no audit events to delete');
      return;
    }
...
async function testHashChain() {
  ...
  try {
    ...
    const expectedHash = createHash('sha256')
      .update(ev.previousHash + payloadStr)
      .digest('hex');
...
async function testTransactionRollback() {
  ...
  try {
    ...
    try {
      await db.transaction(async (tx) => {
        ...
        const recordHash = createHash('sha256')
          .update(previousHash + 'ROLLBACK_TEST')
          .digest('hex');
    ...
    if (countAfter !== countBefore) {
      throw new Error(
        `Count changed: before=${countBefore}, after=${countAfter}. Audit event was NOT rolled back.`,
      );
    }
...
async function testConcurrentAppends() {
  ...
  try {
    ...
    pass(
      `All 5 concurrent events stored with unbroken hash chain (sequences: ${events.map((e) => e.sequenceNumber).join(', ')})`,
    );
  } catch (err) {
    fail('Concurrent appends', err);
  }
}
...
async function testAuthPrincipal() {
  ...
  try {
    ...
    pass(
      'patient.service.ts uses authContext.role and authContext.departmentId — no fabricated values',
    );
  } catch (err) {
    fail('Auth principal truthfulness', err);
  }
}
